...
public class Main {
    ...
    public static String calc(String expr) {
        ...
        try {
            ...
            if ("1".equals(znak))
             {
                throw new Exception("Отсуствует выражение "); // выкидывает, если нет знаков опреции
             }
            ...
            if (!"/".equals(znak)) // делим строку
             ...
             if (StrNumber.length != 2)//  разделилась на три и более строки
              {
                throw new Exception("Формат математической операции не удолетворяет заданию- - два операнда и один оператор (+, -, /, *).");
               }

             if (!RealNum(StrNumber[0].trim(), 1) || !RealNum(StrNumber[1].trim(), 2))//проверка  пробуем перевеси в целые числа
              {
                throw new Exception("Калькулятор умеет работать только с арабскими числами:1,2,3... ");
               }
            arg1 = Integer.parseInt(StrNumber[0].trim()); // первое число
            arg2 = Integer.parseInt(StrNumber[1].trim()); // второе число

            if (arg1 < 1 || arg1 > 10) {
                throw new Exception("Диапозон первого числа должне быть от 1 до 10");
            }
            if (arg2 < 1 || arg2 > 10) {
                throw new Exception("Диапозон второго числа должне быть от 1 до 10");
            }
